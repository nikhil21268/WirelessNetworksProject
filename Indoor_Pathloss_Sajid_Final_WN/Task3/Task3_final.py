...
class RadioMapDataset(Dataset):
    def __init__(self, input_dir, output_dir,npy_dir, buildings, input_transform=None, output_transform=None):
    ...
    def __getitem__(self, idx):
        ...
        npy_file_path = os.path.join(self.npy_dir, self.samples[idx] + '.npy')


        input_image = Image.open(input_image_path).convert('RGB')
        output_image = Image.open(output_image_path).convert('L')

        # Load the 5th channel from the .npy file
        fifth_channel = np.load(npy_file_path)
        fifth_channel = (fifth_channel - fifth_channel.min()) / (fifth_channel.max() - fifth_channel.min())  # Normalize
        fifth_channel = Image.fromarray((fifth_channel).astype('uint8')).convert('L')  # Convert to PIL image
        ...
        if self.output_transform:
            fifth_channel = self.output_transform(fifth_channel)
        ...
        # Calculate the fourth LOS channel
        ...
        input_image = torch.cat((input_image, fourth_channel, fifth_channel), dim=0)
...
input_dir = 'ICASSP2025_Dataset/Inputs/Task_3_ICASSP'
output_dir = 'ICASSP2025_Dataset/Outputs/Task_3_ICASSP'
npy_dir = "Task3_New_Channel_changed_opt"





dataset = RadioMapDataset(input_dir, output_dir, npy_dir, buildings, input_transform = input_transform, output_transform = output_transform)
...
model_save_path = "Task3_unet_aspp_final_model_new.pth"
loss_plot_save_path = "Task3_training_loss_plot_new.png"
...
class RadioMapDataset_Test(Dataset):
    def __init__(self, input_dir,  npy_dir_eval , buildings, input_transform=None):
        self.input_dir = input_dir
        self.output_dir = output_dir
        self.npy_dir = npy_dir
        self.input_transform = input_transform
...
test_dir = "Tasks_Eval_Final/ICASSP_Test_Data/Inputs/Task_3"
npy_dir_eval = "Task3_New_Channel_changed_opt_eval"

# # Test loop
model.eval()

test_dataset = RadioMapDataset_Test(test_dir, npy_dir_eval ,buildings, input_transform = val_test_transform)
...
def save_predictions(dataloader, model, output_dir):
    ...
    with torch.no_grad():
        ...
        print(f"Final Prediction Time for Task3 Eval Data : {total_pred_time:.6f}s")  


output_folder = "Task3_Output_Folder"
