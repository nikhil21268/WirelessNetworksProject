...
model = WNet(in_channels=3, out_channels=1).cuda()
...
plt.savefig('loss_curve_br_da_new.png')
plt.show()  


# Save the model
torch.save(model.state_dict(), 'unet_model_br_da_new.pth')

# # Load the model for inference
# model = UNet().cuda()
model.load_state_dict(torch.load('unet_model_br_da_new.pth'))
model.eval()
...
output_dir = 'predictions_br_da_new'
